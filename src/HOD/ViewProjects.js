...
function HODProjects() {
    let history = useHistory();
    const [empty,setEmpty] = useState("");
    const [project, setProject] = useState([]);
    let localrole = JSON.parse(localStorage.getItem('role'));
    useEffect(() => {
        Axios.post('https://localhost:7273/api/Project/ByHODId', {
    ...
    const updateStatus = async (id,status,uemail,pname) => {
        console.log(uemail);
        if(status===1)
            status="Approved"
        else
                                ...
                                <tbody>
                                    {project.map((item, index = 0) => (
                                        <tr key={item.projectId}>
                                            ...
                                            <td>{item.picid}</td>
                                            <td>{item.igid}</td>
                                            <td><button className='btn btn-success' onClick={(e) => updateStatus(item.projectId,1,item.userId,item.projectName)}>Approve</button>&nbsp;&nbsp;<button className='btn btn-danger' onClick={(e) => updateStatus(item.projectId,0)}>Reject</button>&nbsp;&nbsp;<button className='btn btn-info' onClick={(e) => feedBack(item.projectId,1)}>Feedback</button>&nbsp;&nbsp;</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
